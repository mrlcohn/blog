...
export async function fetchAdminPosts(): Promise<BlogPost[]> {
  try {
    ...
    try {
      ...
      throw new Error(`Admin posts returned ${response.status} but got non-JSON response starting with: "${text.substring(0, 80)}"`);
    }
  } catch (error) {
...
export async function updateAbout(data: {
  ...
  try {
    ...
    throw new Error(`Save returned ${response.status} but got non-JSON response starting with: "${text.substring(0, 80)}"`);
  }
}
